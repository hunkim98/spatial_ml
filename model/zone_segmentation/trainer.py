...
Features:
    - Optional Weights & Biases tracking (auto-enabled when WANDB_API_KEY is set,
      mirrors the pattern used in ac215_Spatially)
    - Mixed-precision training on CUDA (torch.amp)
    - Gradient clipping
    - Per-epoch history written to ``history.json`` for offline plotting
    - Periodic prediction-grid snapshots saved to disk and (when active) to W&B
"""
...
class Trainer:
    ...
    def __init__(
        ...
        # W&B setup — auto-enable if API key is set, mirroring ac215_Spatially.
        ...
        if self.use_wandb:
            ...
            try:
                import wandb  # noqa: F401  (imported lazily inside _init_wandb)

                # Mask the key in the log so we can verify it loaded without leaking it.
    ...
    # ------------------------------------------------------------------ wandb
    def _init_wandb(self, project, run_name, extra_config, lr, weight_decay):
        import wandb

        if run_name is None:
            ts = datetime.now().strftime("%Y%m%d-%H%M%S")
            bs = self.train_loader.batch_size
            run_name = f"unet-film-e{self.epochs}-bs{bs}-{ts}"
    ...
    def train(self) -> dict:
        ...
        if self.use_wandb and self.wandb_run is not None:
            ...
            artifact = wandb.Artifact(
                name="zone-segmentation-model",
                type="model",
                description=f"PatternConditionedUNet best_iou={self.best_iou:.4f}",
            )
