"""Entry point for LOAM-aligned training with vanilla U-Net.

Same LOAM-aligned training params as train_loam.py, but uses VanillaUNet
(no pretrained ResNet backbone) to match LOAM's encoder architecture.
...
def main():
    parser = argparse.ArgumentParser(description="Train zone segmentation (LOAM-aligned, vanilla U-Net)")
    ...
    parser.add_argument("--save-dir", default="checkpoints/zone_seg_loam_vanilla")
    ...
    print("=" * 80)
    print("Zone Segmentation Training (LOAM-aligned, Vanilla U-Net)")
    print("=" * 80)
    print(f"Dataset:      {args.data}")
    print(f"Device:       {args.device}")
    print(f"Model:        VanillaUNet (no pretrained backbone)")
    ...
    # Model — vanilla U-Net, no pretrained weights
    model = VanillaUNet()
    n_params = sum(p.numel() for p in model.parameters()) / 1e6
    logger.info(f"Model: VanillaUNet ({n_params:.1f}M params)")
    ...
    print("\n" + "=" * 80)
    print("Training completed (LOAM-aligned, Vanilla U-Net).")
