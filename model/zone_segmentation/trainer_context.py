"""Training loop for the context-aware U-Net.

Same as trainer_loam.py but handles 4-tuple batches:
(image, pattern, legend, mask) instead of (image, pattern, mask).
"""
...
class TrainerContext:
    """Trains the context-aware U-Net (with legend image input)."""
    ...
    def _init_wandb(self, project, run_name, extra_config, lr, weight_decay, momentum):
        ...
        if run_name is None:
            ...
            run_name = f"unet-context-e{self.epochs}-bs{bs}-{ts}"
        ...
        self.wandb_run = wandb.init(
            ...
            tags=["zone-segmentation", "unet", "context-aware", "spatially"],
        )
        wandb.log({
    ...
    def train_one_epoch(self, epoch: int) -> dict:
        ...
        for batch_idx, (images, patterns, legends, masks) in enumerate(pbar):
            ...
            with torch.amp.autocast(
                ...
                logits = self.model(images, patterns, legends)
    ...
    def train(self) -> dict:
        logger.info(
            f"Training (context-aware): {self.epochs} epochs, "
        ...
        if self.use_wandb and self.wandb_run is not None:
            ...
            artifact = wandb.Artifact(
                name="zone-segmentation-model-context",
                type="model",
                description=f"ContextUNet best_iou={self.best_iou:.4f}",
            )
